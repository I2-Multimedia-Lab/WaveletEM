...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_loop(self, x_in, continous=False):
        ...
        if not self.conditional:
            ...
            ret_img = img.unsqueeze(0)  # 添加时间维度 [1, B, ...]
            
            for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
                img = self.p_sample(img, torch.full(
                    (b,), i, device=device, dtype=torch.long))
                if i % sample_inter == 0:
                    ret_img = torch.cat([ret_img, img.unsqueeze(0)], dim=0)  # 在时间维度上拼接
            return img if not continous else ret_img
        else:
            x = x_in
            shape = x.shape
            b = shape[0]
            img = torch.randn(shape, device=device)
            ret_img = x.unsqueeze(0)  # 添加时间维度 [1, B, ...]
            ...
            if continous:
                return ret_img  # 返回形状 [T, B, ...]
            else:
                return ret_img[-1]  # 返回形状 [1, B, ...]
